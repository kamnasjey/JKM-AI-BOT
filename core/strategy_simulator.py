...
def load_strategy(
    ...
    # Try per-user strategies via the unified store:
    # - dashboard/firebase when configured
    # - local file only when allowed (privacy mode disables local)
    if user_id:
        try:
            from core.user_strategies_store import load_user_strategies

            strategies = load_user_strategies(str(user_id))
            for s in strategies:
